...
class PPO(nn.Module):
    def __init__(self):
        ...
        self.fc_mu = nn.Linear(128, 5)
        self.fc_std = nn.Linear(128, 5)
        self.fc_v = nn.Linear(128, 1)
        self.optimizer = optim.Adam(self.parameters(), lr=learning_rate)
        self.optimization_step = 0
        self.reporter = SummaryWriter('runs/PPO2/forth')
        ...
        if len(self.data) == minibatch_size * buffer_size:
            ...
            for i in range(K_epoch):
                for mini_batch in data:
                    ...
                    loss = -torch.min(surr1, surr2) + 0.9*F.smooth_l1_loss(self.v(s), td_target) - 0.005 * entropy
    ...
    for n_epi in range(10000):
        ...
        while not done:
            for t in range(rollout_len):
                ...
                a = dist.sample()
                log_prob = dist.log_prob(a)
                s_prime, r, done, info = env.step(np.array([a.item() * 0.732 * 1225 + 1225, 0, 0, 0, 0]))
                reward+=r
                rollout.append((s, a, r / 100.0, uavutils.randomstatemapping(s_prime), log_prob.item(), done))
