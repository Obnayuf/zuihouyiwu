...
class SoftQNetwork(nn.Module):
    ...
    def __init__(self, state_size, action_size, hidden_units_1,hidden_units_2, init_w=3e-3):
        ...
        self.fc1 = nn.Linear(state_size + action_size, hidden_units_1)
...
class PolicyNetwork(nn.Module):
    def __init__(self, state_size, action_size, hidden_units_1, hidden_units_2,init_w=3e-5, log_std_min=-20,
        ...
        self.mean_layer = nn.Linear(hidden_units_2, action_size)
        ...
        self.log_std_layer = nn.Linear(hidden_units_2, action_size)
...
class SACAgent:
    def __init__(self, state_size, action_size, alpha,batch_size , buffer_size ,gamma,reward_scale,policy_units_1,policy_units_2,value_units_1,value_units_2,reporter,is_auto_alpha=True, q_lr=LR, policy_lr=LR, a_lr=LR,
        ...
        self.q1_network = SoftQNetwork(self.state_size, self.action_size,hidden_units_1=value_units_1,hidden_units_2=value_units_2).to(device)
        self.q2_network = SoftQNetwork(self.state_size, self.action_size,hidden_units_1=value_units_1,hidden_units_2=value_units_2).to(device)
        self.q1_target = SoftQNetwork(self.state_size, self.action_size,hidden_units_1=value_units_1,hidden_units_2=value_units_2).to(device)
        self.q2_target = SoftQNetwork(self.state_size, self.action_size,hidden_units_1=value_units_1,hidden_units_2=value_units_2).to(device)
        self.policy_network = PolicyNetwork(self.state_size, self.action_size,hidden_units_1=policy_units_1,hidden_units_2=policy_units_2).to(device)
        ...
        # Replay memory
        self.memory = ReplayBuffer(self.buffer_size, self.batch_size, is_action_discrete=False)
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if len(self.memory) > self.batch_size:
            ...
            self.learn(experiences, self.gamma,self.reward_scale)

        # update step count
        self.t_step = (self.t_step + 1) % 2 #更新频率
    ...
    def learn(self, experiences, gamma,scale):
        ...
        self.reporter.add_scalar("mean",rewards.mean(),self.lr_step)
        self.reporter.add_scalar("std",rewards.std(),self.lr_step)
        rewards = (rewards - rewards.mean()) / (rewards.std() + 1e-5)
        rewards = rewards*scale  #reward scale
        ...
        self.reporter.add_scalar("QLOSS", (Q1_loss.mean()+Q2_loss.mean())/2, self.lr_step)
        ...
        if self.t_step == 0:
            ...
            self.reporter.add_scalar("actor_loss",policy_loss,self.lr_step)
    ...
    def train(self, i, n_episodes=10000, max_t=1300):
        ...
        for i_episode in range(1, n_episodes + 1):
            ...
            for t in range(max_t):
                ...
                if done:
                    break
            self.reporter.add_scalar("reward", score, i_episode)
            self.reporter.add_scalar("avgstep", env.stepid, i_episode)
