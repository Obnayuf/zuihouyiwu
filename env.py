...
class DFUAV(object):
    ...
    def reset(self):
        self.time = 0.0
        self.pose = np.array(
            [random.uniform(-5, 5), random.uniform(-5, 5), random.uniform(25, 30),
             random.uniform(-np.pi / 6, np.pi / 6), random.uniform(-np.pi / 6, np.pi / 6),
    ...
    def get_obs(self):
        ...
        obs = np.concatenate(
            (euler, self.get_earth_V(), self.angular_v, np.array([x, y, z, self.time / self.runtime])))  # 15
        obs = uavutils.statemapping(obs)
        return obs

    def Calculate_angle(self, v, vz):
        if v == 0:
            return 0
        if vz == 0:
            if v > 0:
            ...
            if v < 0:
                return -np.pi / 2
        if vz >= 0 and v != 0:
            return -np.arctan(v / vz)
        if vz < 0 and v != 0:
            return -np.arctan(v / vz) + np.pi

    def Calculate_Force(self, speed, c, Vb, omega):  # 螺旋桨推力,speed是螺旋桨的转速
        # duct
        vx,vy,vz = Vb
        Cduct = 0.2  # 单位 m^2
        r = 0.6  # 单位 m
        ...
        g = 9.8
        L1 = 80
        alpha = self.Calculate_angle(vy,vz)
        beta = self.Calculate_angle(vx,vz)
        Cl = max(-Clmax,min(Clmax,0.5 * Cla * np.sin(2*alpha)))
        Cd = Cdgain_1 * (1-np.cos(2*alpha)) + Cd0
        Cy = max(-Cymax,min(Cymax,0.5 * CyB * np.sin(2*beta)))
        Cd_1 = Cdgain_2 * (1 - np.cos(2*beta)) #Cd'
        L = np.array([0, 0.5*])
        Faero = np.array([- 0.5 * Cy * p * (vx**2 + vz**2) * Sside * np.cos(beta) - 0.5 * Cd_1 * p * (vx**2 + vz**2) * Sside * np.sin(beta),
                          0.5 * Cl * p * (vy**2 + vz**2) * Swing * np.cos(alpha) + 0.5 * Cd * p * (vy**2+vz**2) * Swing * np.sin(alpha)])
