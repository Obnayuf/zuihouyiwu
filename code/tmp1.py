...
class PhysicsSim(object):
    def __init__(self,
        ...
        self.set_state_variables(sel_state_variables)

        self.reset()
        self.sv_label = {'pose': ['x', 'y', 'z', 'phi', 'tht', 'psi'],
                         'v': ['Vx', 'Vy', 'Vz'], 'angular_v': ['Vph', 'Vth', 'Vps']}

    # =============================================================================
    #  select state variable, 0th+1st order variables
    # =============================================================================

    def set_state_variables(self, sel_state_variables=None):
        self.sv = ['pose'] if sel_state_variables == None else sel_state_variables

    def get_time(self):
        return self.time

    def get_state_variables(self):
        return np.hstack([self.__dict__[sv] for sv in self.sv])

    def get_state_labels(self):
        return np.hstack([self.sv_label[sv] for sv in self.sv])

    # =============================================================================
    #         return pd.DataFrame( [np.hstack([ self.time, self.pose, self.v, self.angular_v,
    #             self.linear_accel, self.angular_accels]) ],
    #             columns = ['t', 'x', 'y', 'z', 'phi', 'the', 'psi',
    #                  'Vx', 'Vy', 'Vz', 'Vp', 'Vt', 'Vps',
    #                  'Ax', 'Ay', 'Az', 'Aphi', 'Atheta', 'Apsi'])
    #
    # =============================================================================
    ...
    def get_moments(self, thrusts):
        ...
        drag_moment = self.C_d * 0.5 * self.rho * self.angular_v * np.absolute(
            self.angular_v) * self.areas * self.dims * self.dims
    ...
    def next_timestep(self, rotor_speeds):
        ...
        angles = self.pose[
                 3:] + self.angular_v * self.dt + 0.5 * self.angular_accels * self.angular_accels * self.dt ** 2
